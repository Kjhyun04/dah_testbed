...
> - **C2 링크 채널** (지상 GCS ↔ 드론, 양방향 MAVLink) ← R2 대상 (브로드캐스트 RF 매질)
...
`run`은 `dah-gnss` 컨테이너로 상시 가동된다. 수동 디버그(probe/setup/status)는 dahnet
브로드캐스트 세그먼트 안에서 실행해야 하므로 `tools` 컨테이너로 띄운다:

```bash
docker compose exec tools python /work/gnss/gnss_medium.py probe   # (gnss 코드 마운트 시)
# 또는 gnss 컨테이너에서 직접:
docker compose exec gnss python gnss_medium.py status   # 현재 GPS fix 상태
# probe/setup 은 보통 불필요 — m0-baseline.parm 가 부팅 시 외부 GPS로 설정.
```
...
- 브로드캐스트 전환(P-19/P-20): gnss가 **다운링크(14550)에서 SIMSTATE 수신** → **업링크(14555)로
  GPS_INPUT 브로드캐스트** → c2channel이 면제 전달로 air에 주입. `GPS_SOURCE` 스위치로 A1/A2 택일.
...
지상↔UAV **C2/텔레메트리 무선 링크**를 모델링하는 브로드캐스트 매질 허브(`dah-c2channel`, 172.28.0.50).
air(TCP:5760)와 dahnet UDP 브로드캐스트(다운 14550 / 업 14555) 사이를 이으며 MAVLink 메시지 단위
**손실(=재밍)**·지연·failover를 적용한다. **GPS_INPUT(업링크)·SIMSTATE(다운링크)는 면제** —
GPS는 UAV 온보드라 C2 링크와 독립.
