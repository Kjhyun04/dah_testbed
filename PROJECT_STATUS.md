...
- **GPS health 비트 off** (구 verify_all WARN): `GPS_TYPE=14`의 보고 특성. **2026-06-29 Gazebo 통합
  후 정상 전환**(verify_all "GPS health 정상", 8 PASS). prearm 차단도 해소. 비행 시 잔여 issue는
  "Main loop slow"였고 `SCHED_LOOP_RATE=100`으로 해결(GPU 미통과 SW 렌더 CPU 병목). → 비행 실습 가능.
...
### 진행 현황 (2026-06-29 갱신 — 커밋 `e1f0859`)

**A. Gazebo 묶음 — ✅ 완료·검증·커밋.** P-15/P-23/P-24/P-25 구현, baked 이미지 배포 후 기능+현실성
검증 통과(verify_all 8 PASS, 비행 mode→arm→takeoff→land 전주기, failover, 페이로드/영상/noVNC).
상단 「변경 이력 — Gazebo Harmonic 물리엔진 통합」 참조.

**B. arm/GPS-health 게이트 — ✅ 해소.** GPS health는 Gazebo 통합 후 정상 전환, 잔여 "Main loop
slow"는 `SCHED_LOOP_RATE=100`(SW 렌더 CPU 대응)으로 해결. 이륙 LAND 페일세이프는 GPS_INPUT 고도
VFR_HUD 추종으로 해결. → 비행 실습 가능 상태.

### 다음 세션 시작점 — 남은 작업의 Docker 구현 가능성 (중요)
...
## 10. 문서 인덱스
- `README.md` — as-built 실행·사용 가이드 (현행)
- `rf/README.md` · `payload/README.md` · `logviewer/README.md`
- `_excluded/testbed/테스트베드_통합문서.html` — **구성·역할·통신·비행·운용 통합 기준 문서(현행)**
- `_excluded/testbed/테스트베드_가이드.html` — 브로드캐스트+Gazebo 가이드
- `_excluded/testbed/미구현사항_쉬운설명.html` — 미구현 항목 비유 설명
- `../docs/testbed-overview.html` — 개념·설계·STANAG 이론·로드맵
