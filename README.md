...
> **📡 셀룰러(4G/LTE) 트랙 = [`testbed-4g/`](testbed-4g/).** Operation Spiderweb 기반 **셀룰러 C2 드론** 테스트베드
> (Open5GS EPC + srsRAN + ArduPilot SITL, multi-eNB). 전 게이트 실측 통과. **이 브로드캐스트 testbed의 `dah-testbed-air`
> 이미지·`dahnet` 네트워크를 빌려쓰므로** 루트에서 먼저 `docker compose up -d --build` 한 뒤 `testbed-4g/`를 실행.
> 통합 실행·내부기능·아키텍처: [`testbed-4g/docs/testbed-4g_아키텍처_운영가이드.html`](testbed-4g/docs/testbed-4g_아키텍처_운영가이드.html).
